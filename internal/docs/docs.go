// Package docs serves an embedded Swagger UI at /swaggerui so developers can
...
// HandleUI serves the Swagger UI page at GET /swaggerui. The page loads the
// swagger-ui-dist assets from a pinned CDN version and points them at
// HandleSpec (GET /swaggerui/openapi.yaml) to render the document.
...
// GET /swaggerui/openapi.yaml, embedded from api/openapi.yaml at build time.
